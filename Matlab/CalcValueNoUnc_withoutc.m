function vo = CalcValueNoUnc_withoutc(d, c, P, eta, vi, ed, iC, iD)
%%
...
FtED = ((vi/eta).*((vi/eta) > 0) > d); % F_t(v_t(e)/eta + c) 
FtDD = ((vD/eta).*((vD/eta) > 0) > d); % F_t(v_t(e-P/eta)/eta + c) 
% FtED = ((vi/eta) > d); % F_t(v_t(e)/eta + c) 
% FtDD = ((vD/eta) > d); % F_t(v_t(e-P/eta)/eta + c)
