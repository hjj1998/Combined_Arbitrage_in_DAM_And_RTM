function [eF, pF] = Arb_Value_withoutc_v2(lambda,offset,v,e,P,E,eta,c,N)
%%
...
vF(1:(iE-1)) = (vF(1:(iE-1)))/eta; % discharge efficiency

% charge index
iC = find(vF >= (lambda+offset*eta), 1, 'last');

% discharge index
iD = find(vF <= (lambda+offset/eta), 1, 'first');
